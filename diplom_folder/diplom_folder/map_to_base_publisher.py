...
class MapToBasePublisher(Node):
    ...
    def publish_map_to_base(self):
        try:
            # Получаем tag → map из static_transform_publisher
            ...
            # Получаем camera_link → tag из детектора
            ...
            # Получаем base_link → camera_link из URDF
            ...
            # Инвертируем tag → map в map → tag
            map_to_tag_mat = tf_transformations.inverse_matrix(
                self.matrix_from_transform(tag_to_map.transform)
            )
